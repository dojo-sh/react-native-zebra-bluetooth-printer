...
public class RNZebraBluetoothPrinterModule extends ReactContextBaseJavaModule implements ActivityEventListener,BluetoothServiceStateObserver {
    ...
    private byte[] getConfigLabel(ZebraPrinter printer, String label) {
        ...
        try {
            SGD.SET("device.languages", "esc_pos", connection);
    ...
    @ReactMethod
    public void printFromBase64(String device, String base64,final Promise promise) {            //print functionality for zebra printer
...
//        PrinterStatus status = zebraPrinter.getCurrentStatus();
                ...
                byte[] align = {(byte)0x1b, (byte)0x61, (byte)0x01};
                byte[] select = {(byte)0x1b, (byte)0x3d, (byte)0x01};
                byte[] newline = {(byte)0x0a, (byte)0x0d};

                connection.write(newline);
                connection.write(align);
                connection.write(select);

                connection.write(modelQR);

                connection.write(sizeQR);
                connection.write(errorQR);
                connection.write(storeQR);
                connection.write(qrdata.getBytes());
                connection.write(printQR);
                connection.write(newline);
                connection.write(newline);

                String test = new String(modelQR);
